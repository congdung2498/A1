...
public class QuestionController {
    ...
    public ArrayList<Question> getQuestion(int num_exam){
        ...
        Cursor cursor= db.rawQuery("SELECT * FROM tracnghiem WHERE num_exam = '"+num_exam+"' ORDER BY random()",null);
        cursor.moveToFirst();
        do {
            Question item;
            item= new Question(cursor.getInt(0), cursor.getString(1),cursor.getString(2),cursor.getString(3),
                    cursor.getString(4),cursor.getString(5),cursor.getString(7),cursor.getString(8),cursor.getString(9),"","",cursor.getInt(10));
            lsData.add(item);
